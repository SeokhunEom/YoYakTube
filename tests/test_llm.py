...
class TestOpenAIClient:
    ...
    def test_openai_client_initialization(self, mock_openai, api_keys):
        ...
        from yoyaktube.llm import OpenAIClient
    ...
    @patch('yoyaktube.llm.openai.OpenAI')
    def test_openai_client_chat_success(self, mock_openai, api_keys, sample_chat_messages):
        ...
        mock_response.choices[0].message.content = "이 영상은 테스트에 대한 설명을 제공합니다."
    ...
    @patch('yoyaktube.llm.openai.OpenAI')
    def test_openai_client_chat_with_custom_temperature(self, mock_openai, api_keys, sample_chat_messages):
    ...
    def test_openai_client_api_error(self, mock_openai, api_keys, sample_chat_messages):
        ...
        mock_client_instance = Mock()
        mock_client_instance.chat.completions.create.side_effect = openai.OpenAIError("API Error")
        mock_openai.return_value = mock_client_instance
    ...
    @patch('yoyaktube.llm.openai.OpenAI')
    def test_openai_client_authentication_error(self, mock_openai, sample_chat_messages):
        """Test handling authentication error"""
        from yoyaktube.llm import OpenAIClient
        
        mock_client_instance = Mock()
        mock_client_instance.chat.completions.create.side_effect = openai.AuthenticationError("Invalid API key")
        mock_openai.return_value = mock_client_instance
    ...
    @patch('yoyaktube.llm.openai.OpenAI')
    def test_openai_client_rate_limit_error(self, mock_openai, api_keys, sample_chat_messages):
        """Test handling rate limit error"""
        from yoyaktube.llm import OpenAIClient
        
        mock_client_instance = Mock()
        mock_client_instance.chat.completions.create.side_effect = openai.RateLimitError("Rate limit exceeded")
        mock_openai.return_value = mock_client_instance
...
class TestGetOrCreateLlm:
    ...
    def test_create_openai_client(self, mock_openai_client, api_keys):
        ...
        from yoyaktube.llm import get_or_create_llm
...
class TestLlmIntegration:
    ...
    @patch('yoyaktube.llm.openai.OpenAI')
    def test_end_to_end_summarization_flow(self, mock_openai, api_keys, sample_transcript_text):
        ...
        mock_response.choices = [Mock()]
        mock_response.choices[0].message = Mock()
        mock_response.choices[0].message.content = """# 영상 요약
...
- 지속적인 테스트 자동화가 중요"""
    ...
    def test_qa_chat_flow(self, mock_openai, api_keys, sample_transcript_text):
        ...
        mock_response.choices = [Mock()]
        mock_response.choices[0].message = Mock()
        mock_response.choices[0].message.content = "이 영상에서는 단위 테스트, 통합 테스트, 엔드투엔드 테스트 총 3가지 유형의 테스트를 다룹니다."
