...
class TestFetchVideoMetadata:
    ...
    def test_fetch_metadata_success(
        self, mock_yt_dlp, sample_video_id, sample_video_metadata
    ):
        ...
        mock_ydl_instance.extract_info.assert_called_once_with(
            f"https://www.youtube.com/watch?v={sample_video_id}", download=False
        )
    ...
    def test_fetch_metadata_with_minimal_data(self, mock_yt_dlp, sample_video_id):
        ...
        assert result.get("duration") is None or isinstance(
            result.get("duration"), (int, float)
        )
    ...
    def test_fetch_metadata_private_video(self, mock_yt_dlp, sample_video_id):
        ...
        mock_ydl_instance = Mock()
        mock_ydl_instance.extract_info.side_effect = yt_dlp.DownloadError(
            "Private video"
        )
        mock_yt_dlp.return_value.__enter__.return_value = mock_ydl_instance
        mock_yt_dlp.return_value.__exit__.return_value = None
    ...
    @patch("yoyaktube.metadata.yt_dlp.YoutubeDL")
    def test_fetch_metadata_ydl_configuration(
        self, mock_yt_dlp, sample_video_id, sample_video_metadata
    ):
